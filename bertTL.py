...
if __name__ == '__main__':
    ...
    to_encode, labels = load_for_age(DATA_PATH, 'test')
    ...
    # mymodel.save_weights('Transfer_BERT_MTL_lr=0.001_decay2e-2_btch=32_ep2_topicModel.h5')
    mymodel.load_weights(filepath)#, by_name=True)
    layer_output = mymodel.get_layer('encoder_layer').output
    Blog_Embbeder = K.models.Model(inputs=mymodel.input, outputs=layer_output)

    Test = np.zeros((len(test), 64))
    perc = -1
    for i in range(len(test)):
        ...
        if z - perc >= 1:
            perc = z
            print('Encoding ', str(perc) + ' %\r', end="")

        Test[i] = Blog_Embbeder.predict([np.array([test[i]]), np.array([seg_input[i]]),
                                            np.array([mask_input[i]])])

    np.save('./data/Bert_encode.npy', Test)
    os.system('clear')
    print('Bert Representation Extracted Successful !!!')